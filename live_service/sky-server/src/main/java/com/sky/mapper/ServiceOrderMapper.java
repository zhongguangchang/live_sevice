...
public interface ServiceOrderMapper {
    ...
    /**
     * 查询某师傅在某时段已占用的订单（派单时判断师傅档期是否冲突）
     */
    @Select("select count(id) from service_order where provider_id = #{providerId} and slot_id = #{slotId} " +
            "and status in (2, 3, 4, 5)")
    Integer countProviderBusy(@Param("providerId") Long providerId, @Param("slotId") Long slotId);
